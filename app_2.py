...
def create_auto_position(strike, option_type, buy_price, qty, symbol='NIFTY', expiry=''):
    ...
    # 🚨 CRITICAL: This becomes the absolute minimum - NEVER go below this!
...
def execute_auto_sell(position, reason='Stop Loss'):
    ...
    if app_state['paper_trading_enabled']:
        ...
        trade = {
            ...
            'time': dt.now().strftime('%Y-%m-%d %H:%M:%S'),  # Add formatted time
            'reason': reason
        }
    ...
    if 'Stop Loss' in reason:
        ...
        position['last_stop_loss_price'] = position.get('stop_loss_price', sell_price)
        ...
        # 🔥 DEBUGGING: Print auto buy setup details
        print(f"🎯 AUTO BUY SETUP: Will trigger when price reaches ₹{position['last_stop_loss_price']} (where it was sold)")
        print(f"📊 Position waiting for auto buy: Strike {position['strike']} {option_type} | Quantity: {position['original_quantity']}")

        # 🚨 FIXED: In paper trading, keep position in paper_positions for auto-buy monitoring
        # Don't add to auto_positions, keep it in paper_positions
    ...
    if 'Stop Loss' in reason:
        ...
        # 🔥 DEBUGGING: Print auto buy setup details for live trading
        print(f"🎯 LIVE AUTO BUY SETUP: Will trigger when price reaches ₹{position['last_stop_loss_price']} (where it was sold)")
        print(f"📊 Live position waiting for auto buy: Strike {position['strike']} {position['type']}")
...
def execute_auto_buy(position):
    ...
    if app_state['paper_trading_enabled']:
        ...
        # 🚨 USER REQUIREMENT: Auto buy ka stop loss = auto sell price (same where it was sold)
        # NOT auto buy price - 10, but the original sell price
        position['stop_loss_price'] = position.get('last_stop_loss_price', buy_price - 10)
        ...
        position['timestamp'] = dt.now().isoformat()
    ...
    print(f"📍 AUTO BUY EXECUTED: {position['strike']} {position['type']} @ ₹{buy_price} | Stop Loss: ₹{position['stop_loss_price']} (Same as auto sell price)")
    ...
    print(f"🟢 AUTO BUY: {position['strike']} {position['type']} @ ₹{buy_price} | Stop Loss: ₹{position['stop_loss_price']} (Same as auto sell price) | {order_status}")
    return True

def update_trailing_stop_loss(position):
    """SIMPLE 10-POINT TRAILING STOP LOSS - MOVES IN 10 RUPEE STEPS ABOVE BUY PRICE
    ...
    if position.get('auto_bought'):
        ...
        if profit_from_auto_buy >= trailing_step:  # If profit >= 10
            ...
            # 🚨 CORRECTED FORMULA: Stop loss = auto buy price + (completed steps × 10)
            # Example: Auto buy ₹160, highest ₹170 → 1 step → Stop loss = ₹160 + ₹10 = ₹170
            # Example: Auto buy ₹160, highest ₹180 → 2 steps → Stop loss = ₹160 + ₹20 = ₹180
            new_trailing_stop_loss = auto_buy_price + (profit_steps * trailing_step)  # Step-based calculation
            ...
            print(f"🎯 AUTO BUY TRAILING: Auto Buy ₹{auto_buy_price} | High ₹{highest_after_auto_buy} | Profit ₹{profit_from_auto_buy:.2f} | Steps: {profit_steps}")
            print(f"   STEP LOGIC: Stop Loss = Auto Buy Price + (Steps × 10) = ₹{auto_buy_price} + ({profit_steps} × 10) = ₹{new_trailing_stop_loss:.2f}")
            ...
            print(f"🔒 AUTO BUY INITIAL STOP LOSS: Auto Buy ₹{auto_buy_price} | High ₹{highest_after_auto_buy} | Profit ₹{profit_from_auto_buy:.2f} < ₹{trailing_step} (No trailing yet)")
            print(f"   Stop Loss: ₹{position['stop_loss_price']:.2f} (Auto buy price - ₹{stop_loss_point})")
    else:
        # Manual buy: FIXED 10-POINT TRAILING - Stop loss moves in 10 rupee steps from buy price
        ...
        if profit >= trailing_step:  # If profit >= 10
            ...
            # 🚨 CORRECTED FORMULA: Stop loss = buy price + (completed steps × 10)
            # Example: Buy ₹117, High ₹127 → 1 step → Stop loss = ₹117 + ₹10 = ₹127
            # Example: Buy ₹117, High ₹137 → 2 steps → Stop loss = ₹117 + ₹20 = ₹137  
            # Example: Buy ₹117, High ₹147 → 3 steps → Stop loss = ₹117 + ₹30 = ₹147
            new_trailing_stop_loss = original_buy_price + (profit_steps * trailing_step)  # Step-based calculation
            ...
            print(f"🎯 MANUAL TRAILING: Buy ₹{original_buy_price} | High ₹{highest_price} | Profit ₹{profit:.2f} | Steps: {profit_steps}")
            print(f"   STEP LOGIC: Stop Loss = Buy Price + (Steps × 10) = ₹{original_buy_price} + ({profit_steps} × 10) = ₹{new_trailing_stop_loss:.2f}")
            print(f"   Old Stop Loss: ₹{old_stop_loss:.2f} → New Stop Loss: ₹{position['stop_loss_price']:.2f}")
            
        else:
            # Default: EXACTLY 10 points below buy price (initial stop loss)
            ...
            print(f"🔒 INITIAL STOP LOSS: Buy ₹{original_buy_price} | High ₹{highest_price} | Profit ₹{profit:.2f} < ₹{trailing_step} (No trailing yet)")
            print(f"   Stop Loss remains: ₹{position['stop_loss_price']:.2f} (Buy price - ₹{stop_loss_point})")
        ...
        for position in positions_to_monitor[:]:  # Create copy to allow removal during iteration
            ...
            for option in all_options:
                try:
                    ...
                    # 🔥 DEBUG: Print option matching details for waiting positions
                    if position.get('waiting_for_autobuy', False):
                        print(f"🔍 MATCHING CHECK: Position {position_strike} {position_type} vs Option {option_strike} {option_type}")
            ...
            if matching_option:
                try:
                    ...
                    if position.get('waiting_for_autobuy', False):
                        ...
                        if abs(current_price - auto_buy_trigger_price) <= max(1, auto_buy_buffer):
                            print(f"🟢 AUTO BUY TRIGGERED: Price ₹{current_price} reached trigger ₹{auto_buy_trigger_price} (Stop loss sell price ± buffer)")
                        ...
                        if app_state['paper_trading_enabled']:
                            ...
                            # Update trailing stop loss with new price data
                            update_trailing_stop_loss(position)
                            ...
                            # 🚨 FIXED: Sell if current price drops below stop loss (regardless of profit/loss)
                            # This allows trailing stop loss to work in profit situations too
                            if (current_price <= stop_loss_price and  # Price dropped to or below stop loss (same as live trading)
                ...
                if not position.get('waiting_for_autobuy', False):
                    ...
                    if app_state['paper_trading_enabled']:
                        ...
                        # 🚨 FIXED: Sell if current price drops below stop loss (Last Price)
                        if (current_price <= stop_loss_price and  # Price dropped to or below stop loss (same as live trading)
                            stop_loss_price > 0 and 
                            not position.get('sell_triggered', False)):
...
def api_expiry_list(symbol):
    ...
    headers = {
        ...
        "authorization": "Bearer j-7_HKLdVVLOj_xq-D0_CtoaPrLDXY0TYq1Fk9wxr5yaCxTWHwZ4ucS14TUlAAcBT5J-ur2YqyFL8EZMZeUlGRjfftjqCqyZLo-Xi02P3W7akSeK3vIjKisp7Rpt3zA4P8wxZn3EwQuB5lb_cUbtb7-IYnVOIESs2Bwl626KBhsTZjoqaau8iVPO6T1JtILG8A_ath4qjUt7y5FLzt3UbRovsYgo7ab_jhb485XgCzCCs135Q4FDCMFVWDyUqPvg4OiG7QalIjuS8cWUbD4yLQ"
    }
...
def paper_buy_option(strike, option_type, price, quantity, lot_size):
    ...
    position = {
        ...
        'pnl_percentage': 0.0,
        # Auto-trading fields
        'stop_loss_price': price - 10,  # 10-point stop loss
        'original_buy_price': price,
        'highest_price': price,  # Initialize highest price to buy price
        'minimum_stop_loss': price - 10,  # Minimum stop loss buffer
        'auto_buy_count': 0,
        'auto_sell_count': 0,
        'total_pnl': 0.0,
    ...
    print(f"📄 Paper BUY: {quantity} lot(s) of {option_type} {strike} @ ₹{price:.2f} = ₹{total_cost:.2f} (Auto-monitoring enabled)")
    ...
    if app_state['paper_trading_enabled']:
        ...
        for pos in app_state['paper_positions']:
            ...
            if current_option_data:
                for option_type_data in ['calls', 'puts', 'atm']:
                    if option_type_data in current_option_data:
                        for option in current_option_data[option_type_data]:
                            if (option.get('strike') == pos['strike'] and 
                                option.get('option_type') == pos['option_type']):
                                current_price = option.get('ltp', current_price)
                                break
            ...
            position_data = {
                ...
                'id': pos['id'],
                'mode': 'paper',
                'stop_loss_price': pos.get('stop_loss_price', pos['buy_price'] - 10),  # Include stop loss
                ...
                'status': 'Pending Auto-Buy' if pos.get('waiting_for_autobuy', False) else 'Active'
            }
...
def api_auto_start_option_chain():
    ...
    try:
        ...
        headers = {
            "accept": "application/json",
            "authorization": "Bearer QF9gz6jrWBu2rP_cgEzzn2cF9nuLSBR3isUhJM_KXJYy8QG8y7gO05IdurfYBttC98nOPk9PEYul9m9GRqtYg_0jjWrdZd-sy5BfKU4kJmWu05LVOvYNKjAIlWJm8B9Jvi-Nj_xxJDKR4zwp1Ez8or1Aq8CnRX6P29bt5xTUUDwcGCjRN9DMmoLSB6bvPijFfOs9EhExskgnYifSUYa7X64OBYh-vkD2ogItT1seqn8HTtgJ4ZFj0TFw0T7iTj0EqfV07CT-DMXxG22CqWwvxA"
        }
